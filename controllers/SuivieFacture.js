...
const { SuivieFacture } = require("../database/SuivieFactureQuery");
exports.getallCountexport = async (req, res) => {
  try {
    const pool = await getConnection();
    const result = await pool
      .request()
      .query(SuivieFacture.getSuivieFacturecount);
...
exports.getSuivieFacture = async (req, res) => {
  try {
    ...
    const result = await pool.request().query(
      `${SuivieFacture.getSuivieFacture} ${queryFilter} Order by ${sort[0]} ${
        sort[1]
      }
...
exports.getSuivieFactureCountEchu = async (req, res, next) => {
  try {
    ...
    const result = await pool
      .request()
      .query(SuivieFacture.getSuivieFactureEchucount);
    req.count = result.recordset[0].count;
    next();
  } catch (error) {
    res.status(500);
...
exports.getSuivieFactureEchu = async (req, res) => {
  try {
    ...
    const pool = await getConnection();
    const result = await pool.request().query(
      `${SuivieFacture.getSuivieFactureEchu} ${queryFilter} Order by ${
        sort[0]
      } ${sort[1]}
                OFFSET ${range[0]} ROWS FETCH NEXT ${
        range[1] + 1 - range[0]
...
exports.getSuivieFactureNonPayeCount = async (req, res, next) => {
  try {
    ...
    const result = await pool
      .request()
      .query(SuivieFacture.getSuivieFactureNonPayéCount);
    filter = JSON.parse(filter);
    console.log(filter);
    let queryFilter = "";
    if (filter.annee) {
...
exports.getSuivieFactureNonPayé = async (req, res) => {
  try {
    ...
    const pool = await getConnection();
    const result = await pool.request().query(
      `${SuivieFacture.getSuivieFactureNonPayé} ${queryFilter} Order by ${
        sort[0]
      } ${sort[1]}
              OFFSET ${range[0]} ROWS FETCH NEXT ${
        range[1] + 1 - range[0]
...
exports.getAnneeFacture = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query(`${SuivieFacture.getAnneSuivieFacture}`);

    res.set("Content-Range", `SuivieFacture`);
...
exports.SuivieFactureByFournisseurExercice = async (req, res) => {
  try {
    const pool = await getConnection();
    const result = await pool
      .request()
